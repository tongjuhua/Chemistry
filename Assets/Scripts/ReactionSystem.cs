...
public class ReactionSystem : MonoBehaviour
{
	...
	void FixedUpdate()
	{
		...
		foreach (string rTag in reactions.Keys)
		{
           ...
           // Debug.Log(2);
			...
			foreach (substanceInfoOfReaction sir in rctInfo.reactants)
			{
				...
				reactInfo += sir.rate + sir.name + "(" + substance[sir.name].amount[(int)sir.type].ToString("f2") + " mol";
				if (sir.type == substanceType.Liquid)
				{
					substance[sir.name].concentration += reactionAmount / volumn;
					reactInfo += ", " + substance[sir.name].concentration.ToString("f2") + " mol/L";
				}
				reactInfo += ") + ";
			}
            //Debug.Log(4);
			...
			foreach (substanceInfoOfReaction sir in rctInfo.products)
			{
                ...
                {
                    reactInfo += "(" + substance[sir.name].amount[(int)sir.type].ToString("f2") + " mol";
                    if (sir.type == substanceType.Liquid)
                    {
                        substance[sir.name].concentration += reactionAmount / volumn;
                        reactInfo += ", " + substance[sir.name].concentration.ToString("f2") + " mol/L";
        ...
        UCE.TipBoard.SetText(reactInfoTotal);
	}
	...
	GameObject CreateNewSubstance(substanceType type, Transform t, Mesh mesh, Material mat, float[] color)
	{
        ...
        if (type == substanceType.Gas && sysType == systemType.Solution)
            new_obj.GetComponent<Bubbles>().IsInSolution = true;
		return new_obj;
	}
